...
export function BubbleMenuNodeSelector(props: Props) {
  ...
  return (
    ...
    >
      <section className="mt-1 flex max-h-[90vh] w-48 flex-col overflow-y-scroll rounded-lg border-[0.5px] border-strong bg-surface-1 px-2 py-1.5 shadow-raised-200">
        {items.map((item) => (
          <button
            key={item.name}
            type="button"
            ...
            className={cn(
              "flex items-center justify-between rounded-lg px-1 py-1 text-13 text-secondary hover:bg-layer-1",
              {
