  ...
  return (
    <>
      ...
      <div
        ...
        className={cn(
          "invisible relative max-h-80 w-[14rem] space-y-1 overflow-y-auto rounded-lg border-[0.5px] border-strong bg-surface-1 px-2 py-1.5 opacity-0 shadow-raised-200 transition-opacity",
          {
      ...
      >
        {items.length ? (
          items.map((item, index) => {
            ...
            return (
              <button
                ...
                type="button"
                className={cn(
                  "flex w-full items-center gap-2 truncate rounded-lg px-2 py-1 text-left text-13 text-secondary transition-colors duration-150 hover:bg-layer-1-hover",
                  {
          ...
          <div className="py-1.5 text-center text-13 text-placeholder">No emojis found</div>
        )}
      </div>
    </>
  );
