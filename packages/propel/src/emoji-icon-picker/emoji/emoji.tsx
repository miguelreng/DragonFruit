...
export function EmojiRoot(props: EmojiRootProps) {
  ...
  return (
    <EmojiPicker.Root
      ...
      className="isolate flex h-full w-full flex-col rounded-lg border-none p-2"
      ...
      <div className="sticky top-0 z-10 flex items-center justify-between gap-2 bg-surface-1 px-1.5 py-2 [&>[data-slot='emoji-picker-search-wrapper']]:flex-grow [&>[data-slot='emoji-picker-search-wrapper']]:p-0">
        <div ref={searchWrapperRef} data-slot="emoji-picker-search-wrapper" className="p-2">
          <EmojiPicker.Search
            ...
            className="block h-full w-full flex-grow-0 rounded-lg border-[0.5px] border-subtle bg-transparent p-0 px-3 py-2 text-13 placeholder-(--text-color-placeholder) focus:border-accent-strong focus:outline-none"
          />
        </div>
        <EmojiPicker.SkinToneSelector
          data-slot="emoji-picker-skin-tone-selector"
          className="hover:bg-accent mx-2 mb-1.5 size-8 flex-shrink-0 rounded-lg bg-surface-1 text-16"
        />
      </div>
          ...
          components={{
            ...
            Emoji: ({ emoji, ...props }) => (
              <button
                ...
                className="data-active:bg-accent flex size-8 items-center justify-center rounded-lg text-16"
                {...props}
              >
                {emoji.emoji}
              </button>
