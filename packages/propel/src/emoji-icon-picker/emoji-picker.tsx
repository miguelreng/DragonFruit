          ...
          <Tabs.List className="grid grid-cols-2 gap-1 px-3.5 pt-3">
            {tabs.map((tab) => (
              <Tabs.Tab
                ...
                value={tab.key}
                className={({ selected }) =>
                  cn("rounded-lg border border-subtle bg-layer-1 py-1 text-13", {
        ...
        {onRemove && hasValue && (
          <div className="flex items-center justify-end border-t border-subtle bg-surface-1 px-3 py-2">
            <button
              ...
              className="rounded-lg px-2 py-1 text-13 font-medium text-danger-primary transition-colors hover:bg-danger-primary/10"
            >
              Remove
            </button>
          </div>
