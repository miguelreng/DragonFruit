...
import { CheckIcon } from "@dragonfruit/propel/icons";
// helpers
...
export function DropdownOptions(props: IMultiSelectDropdownOptions | ISingleSelectDropdownOptions) {
  ...
  return (
    <>
      ...
      <div className={cn("max-h-48 space-y-px overflow-y-scroll", !disableSearch && "mt-1")}>
        <>
          {options ? (
            options.length > 0 ? (
              options?.map((option) => (
                <Combobox.Option
                  ...
                  className={({ active, selected }) =>
                    cn(
                      "flex w-full cursor-pointer items-center justify-between gap-2 truncate rounded-sm px-1 py-1 select-none",
                      {
                        "bg-layer-1": active,
