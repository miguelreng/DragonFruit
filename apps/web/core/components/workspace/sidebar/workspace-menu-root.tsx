...
function WorkspaceMenuOpenSync({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  useEffect(() => {
    onOpenChange(open);
...
export const WorkspaceMenuRoot = observer(function WorkspaceMenuRoot(props: WorkspaceMenuRootProps) {
  ...
  return (
    ...
    >
      {({ open }: { open: boolean }) => (
        <>
          <WorkspaceMenuOpenSync open={open} onOpenChange={toggleAnySidebarDropdown} />
          {variant === "sidebar" && (
          ...
          {variant === "top-navigation" && (
            ...
            >
              <div className="flex flex-grow items-center gap-2 truncate">
                ...
                />
                <h4 className="truncate text-14 font-semibold text-secondary">
                  {activeWorkspace?.name ?? t("loading")}
                </h4>
              </div>
              <ChevronDownIcon
