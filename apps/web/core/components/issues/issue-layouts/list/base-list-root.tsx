...
// plane constants
import { EIssueFilterType, EUserPermissions, EUserPermissionsLevel } from "@dragonfruit/constants";
// types
import type { EIssuesStoreType, GroupByColumnTypes, TGroupedIssues, TIssueKanbanFilters } from "@dragonfruit/types";
    ...
    <IssueLayoutHOC layout={EIssueLayoutTypes.LIST}>
      <div className={`relative size-full bg-surface-2`}>
        <List
          ...
          isEpic={isEpic}
        />
        {/* Floating bulk-action bar; renders only when one or more tasks
            are selected. Lives at the list-root level so it stays anchored
            to the bottom of the viewport regardless of list scroll position. */}
