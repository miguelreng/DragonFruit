...
// plane imports
import { ROLE, EUserPermissions } from "@dragonfruit/constants";
import { useTranslation } from "@dragonfruit/i18n";
import { Button } from "@dragonfruit/propel/button";
import { PlusIcon, CloseIcon, ChevronDownIcon } from "@/components/icons/propel-shim";
import { TOAST_TYPE, setToast } from "@dragonfruit/propel/toast";
import { Avatar, CustomSelect, CustomSearchSelect, EModalPosition, EModalWidth, ModalCore } from "@dragonfruit/ui";
// helpers
    ...
    <ModalCore isOpen={isOpen} handleClose={handleClose} position={EModalPosition.CENTER} width={EModalWidth.XXL}>
      <form onSubmit={handleSubmit(onSubmit)} className="p-5">
        <div className="space-y-5">
          <h3 className="text-16 leading-6 font-normal text-primary">
                    ...
                    render={({ field: { value, onChange } }) => {
                      ...
                      return (
                        <CustomSearchSelect
                          value={value}
                          customButton={
                            <button type="button" className="shadow-sm flex w-full items-center justify-between gap-1 rounded-lg border border-subtle px-3 py-2 text-left text-13 text-secondary duration-300 hover:bg-layer-1 hover:text-primary focus:outline-none">
                              {value && value !== "" ? (
