...
export function AgentsList({ agents, onToggle, onUpdateTrigger }: IAgentsListProps) {
  return (
    <div className="flex w-full flex-col divide-y divide-subtle overflow-hidden rounded-lg border border-subtle bg-layer-2">
      {agents.map((agent) => (
        <AgentsListItem key={agent.id} agent={agent} onToggle={onToggle} onUpdateTrigger={onUpdateTrigger} />
      ))}
    </div>
  );
}
