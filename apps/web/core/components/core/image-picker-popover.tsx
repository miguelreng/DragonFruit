...
// plane imports
import { ACCEPTED_COVER_IMAGE_MIME_TYPES_FOR_REACT_DROPZONE, MAX_FILE_SIZE } from "@dragonfruit/constants";
import { useOutsideClickDetector } from "@dragonfruit/hooks";
import { Tabs } from "@dragonfruit/propel/tabs";
import { Button, getButtonStyling } from "@dragonfruit/propel/button";
import { TOAST_TYPE, setToast } from "@dragonfruit/propel/toast";
import { EFileAssetType } from "@dragonfruit/types";
import { Input, Loader } from "@dragonfruit/ui";
// helpers
...
export const ImagePickerPopover = observer(function ImagePickerPopover(props: Props) {
  ...
  return (
    <Popover className="relative z-19" ref={ref} tabIndex={tabIndex} onKeyDown={handleKeyDown}>
      ...
      {isOpen && (
        <Popover.Panel
          className="absolute right-0 z-20 mt-2 rounded-lg border border-subtle bg-surface-1 shadow-raised-200"
          static
        >
          <div
            ref={imagePickerRef}
                            ...
                            {unsplashImages.map((image) => (
                              ...
                              >
                                <img
                                  ...
                                  className="absolute top-0 left-0 h-full w-full cursor-pointer rounded-lg object-cover"
                                />
                              </div>
                            ))}
                          </div>
                    ...
                    {Object.values(STATIC_COVER_IMAGES).map((imageUrl, index) => (
                      ...
                      >
                        <img
                          ...
                          className="absolute top-0 left-0 h-full w-full cursor-pointer rounded-lg object-cover transition-opacity hover:opacity-80"
                        />
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="flex h-full w-full flex-col gap-y-2">
                    <div className="flex w-full flex-1 items-center gap-3">
                      ...
                      >
                        <button
                          type="button"
                          className="absolute top-0 right-0 z-40 -translate-y-1/2 rounded-lg bg-surface-2 px-2 py-0.5 text-11 font-medium text-secondary"
                        >
                          Edit
                        </button>
