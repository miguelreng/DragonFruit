...
type Props = {
  ...
  onClose: () => void;
  onSaved: (template: TPageTemplateDetail) => void;
};

export function PageTemplateModal({ isOpen, workspaceSlug, template, onClose, onSaved }: Props) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const saved = isEdit
        ...
        : await templateService.create(workspaceSlug, payload);
        ...
        <div className="flex flex-col gap-4 px-5 py-4">
          ...
          <label className="flex flex-col gap-1.5">
            ...
            <input
              ...
              placeholder="What this doc template is for"
              className="rounded-lg border-[0.5px] border-subtle bg-layer-1 px-3 py-2 text-13 text-primary outline-none placeholder:text-placeholder focus:border-strong"
            />
          </label>
