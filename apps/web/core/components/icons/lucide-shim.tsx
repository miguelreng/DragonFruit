 ...
 *
 * Lucide -> HugeIcons shim.
 *
 * The codebase imports icons by Lucide names; this shim re-exports each name
 * as a thin wrapper around a matching HugeIcons (stroke-rounded) icon, so the
 * 200+ call sites don't have to change when we evolve the icon library. If a
 * name is missing here, add it: find the closest match at
 * https://hugeicons.com/icons and alias it below.
 */
 ...
 * shape callers used for the previous Phosphor exports (className, color, etc.)
 * — drop-in replacement so the ~200 importers stay unchanged.
 */
...
export const Code2 = huge(CodeIcon);
...
export const Code = huge(CodeIcon);
export const Table = huge(TableIcon);
