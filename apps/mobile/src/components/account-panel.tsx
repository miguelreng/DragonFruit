import { useState } from "react";
import { Alert, Pressable, ScrollView, StyleSheet, Text, View } from "react-native";
import Constants from "expo-constants";

import { AppIcon } from "@/components/app-icon";
import { Avatar } from "@/components/avatar";
import { PressableScale } from "@/components/pressable-scale";
import { ScrollFade } from "@/components/scroll-fade";
import { WorkspaceSwitcher } from "@/components/workspace-switcher";
import type { Workspace } from "@/lib/api";
import { ArrowDown01Icon, ArrowRight01Icon, GlobeIcon, Logout03Icon } from "@/lib/icons";
import { openWeb } from "@/lib/open-web";
import { useSession } from "@/lib/session";
import { colors, font, radius, shadow, spacing } from "@/lib/theme";

/**
 * The hub's Account tab. Holds what used to live in the workspace header — a
 * full-width dropdown to switch workspace and the signed-in user — plus the
 * profile actions (open web, sign out).
 */
export function AccountPanel({
  workspaces,
  currentSlug,
  onSelectWorkspace,
}: {
  workspaces: Workspace[];
  currentSlug: string;
  onSelectWorkspace: (slug: string) => void;
}) {
  const { user, signOut } = useSession();
  const displayName = user?.display_name || user?.first_name || user?.email || "You";
  const version = Constants.expoConfig?.version ?? "1.0.0";
  const [switcherOpen, setSwitcherOpen] = useState(false);

  const current = workspaces.find((w) => w.slug === currentSlug);
  const currentName = current?.name ?? currentSlug.replace(/-/g, " ");

  const toggleSwitcher = () => {
    setSwitcherOpen(true);
  };

  const pickWorkspace = (slug: string) => {
    if (slug !== currentSlug) onSelectWorkspace(slug);
  };

  const confirmSignOut = () => {
    Alert.alert("Sign out", "You'll need to sign in again to use DragonFruit.", [
      { text: "Cancel", style: "cancel" },
      { text: "Sign out", style: "destructive", onPress: () => void signOut() },
    ]);
  };

  return (
    <View style={styles.root}>
      <View style={styles.headerRow}>
        <Text style={styles.pageTitle}>Account</Text>
      </View>

      <ScrollFade topHeight={20} bottomHeight={80}>
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          <View style={styles.identity}>
            <Avatar name={displayName} size={72} circle imageUrl={user?.avatar_url || user?.avatar} />
            <Text style={styles.name} numberOfLines={1}>
              {displayName}
            </Text>
            {user?.email ? (
              <Text style={styles.email} numberOfLines={1}>
                {user.email}
              </Text>
            ) : null}
          </View>

          {/* Full-width trigger — the workspace list opens in the shared animated drawer. */}
          <PressableScale
            onPress={toggleSwitcher}
            accessibilityRole="button"
            accessibilityState={{ expanded: switcherOpen }}
            accessibilityLabel={`Switch workspace, currently ${currentName}`}
          >
            {({ pressed }) => (
              <View style={[styles.dropdownTrigger, pressed && styles.rowPressed]}>
                {/* Keep the surface on a concrete View so New Architecture renders its white background reliably. */}
                <View style={styles.dropdownTriggerInner}>
                  <Avatar name={currentName} size={32} imageUrl={current?.logo_url || current?.logo} />
                  <Text style={styles.dropdownTriggerLabel} numberOfLines={1}>
                    {currentName}
                  </Text>
                  <AppIcon icon={ArrowDown01Icon} size={18} color={colors.textTertiary} strokeWidth={2} />
                </View>
              </View>
            )}
          </PressableScale>

          <View style={styles.card}>
            <Pressable onPress={() => openWeb("/")} accessibilityRole="button" accessibilityLabel="Open the web app">
              {({ pressed }) => (
                <View style={[styles.row, pressed && styles.rowPressed]}>
                  <AppIcon icon={GlobeIcon} size={18} color={colors.body} strokeWidth={1.9} />
                  <Text style={styles.rowLabel}>Open web app</Text>
                  <AppIcon icon={ArrowRight01Icon} size={16} color={colors.faint} strokeWidth={1.9} />
                </View>
              )}
            </Pressable>
          </View>

          <Pressable onPress={confirmSignOut} accessibilityRole="button" accessibilityLabel="Sign out">
            {({ pressed }) => (
              <View style={[styles.signOut, pressed && styles.signOutPressed]}>
                <AppIcon icon={Logout03Icon} size={18} color={colors.danger} strokeWidth={1.9} />
                <Text style={styles.signOutText}>Sign out</Text>
              </View>
            )}
          </Pressable>

          <Text style={styles.version}>DragonFruit for iOS · v{version}</Text>
        </ScrollView>
      </ScrollFade>

      <WorkspaceSwitcher
        visible={switcherOpen}
        workspaces={workspaces}
        currentSlug={currentSlug}
        onSelect={pickWorkspace}
        onClose={() => setSwitcherOpen(false)}
      />
    </View>
  );
}

const styles = StyleSheet.create({
  root: { flex: 1 },
  headerRow: { paddingHorizontal: spacing.lg, paddingTop: spacing.lg, paddingBottom: spacing.sm },
  pageTitle: { color: colors.ink, fontFamily: "Figtree_600SemiBold", fontSize: font.size.xxl, letterSpacing: -0.45 },
  content: { paddingHorizontal: 20, paddingBottom: 32 },
  identity: { alignItems: "center", paddingVertical: spacing.lg },
  name: { marginTop: spacing.md, fontSize: font.size.xl, color: colors.ink, fontFamily: "Figtree_700Bold" },
  email: { marginTop: 2, fontSize: font.size.sm, color: colors.muted, fontFamily: "Figtree_400Regular" },
  // Full-width workspace dropdown trigger.
  dropdownTrigger: {
    borderRadius: radius.lg,
    borderWidth: 1,
    borderColor: colors.border,
    backgroundColor: colors.white,
    paddingHorizontal: spacing.md,
    paddingVertical: 10,
    ...shadow.card,
  },
  dropdownTriggerInner: { flexDirection: "row", alignItems: "center", gap: spacing.md },
  dropdownTriggerLabel: { flex: 1, fontSize: font.size.md, color: colors.ink, fontFamily: "Figtree_600SemiBold" },
  card: {
    marginTop: spacing.lg,
    borderRadius: radius.lg,
    borderWidth: 1,
    borderColor: colors.border,
    backgroundColor: colors.surface,
    ...shadow.card,
  },
  row: {
    flexDirection: "row",
    alignItems: "center",
    gap: spacing.md,
    paddingHorizontal: spacing.md,
    paddingVertical: spacing.md,
    minHeight: 52,
  },
  rowPressed: { backgroundColor: colors.layerTransparentHover },
  rowLabel: { flex: 1, fontSize: font.size.sm, color: colors.ink, fontFamily: "Figtree_500Medium" },
  signOut: {
    marginTop: spacing.lg,
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "center",
    gap: spacing.sm,
    borderRadius: radius.md,
    borderWidth: 1,
    borderColor: colors.border,
    backgroundColor: colors.surface,
    paddingVertical: spacing.md,
  },
  signOutPressed: { backgroundColor: colors.dangerSoft },
  signOutText: { fontSize: font.size.md, color: colors.danger, fontFamily: "Figtree_600SemiBold" },
  version: {
    marginTop: spacing.lg,
    textAlign: "center",
    fontSize: font.size.xs,
    color: colors.faint,
    fontFamily: "Figtree_400Regular",
  },
});
